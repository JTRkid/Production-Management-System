...
| 路由 | Vue Router | 4.6 |
| 图表 | ECharts | 5.x |
| Web 服务器 | Nginx（反向代理 + 静态资源） | alpine |
| ASGI 服务器 | Gunicorn + Uvicorn Worker | — |
...
┌─────────────────────────────────────────────────────┐
│                    Nginx (:80 / :443)                │
│  / → Vue 3 SPA    /api/ → 后端    /ws/ → WebSocket  │
├─────────────────────────────────────────────────────┤
│         Gunicorn + Uvicorn (Django REST Framework)   │
│         JWT 认证 · django-filters · Channels         │
├────────────┬────────────┬───────────────────────────┤
│  基础数据   │   生产管理   │        报表分析            │
├────────────┴────────────┴───────────────────────────┤
│  SQLite（开发）/ MySQL 8.0（生产） │ Redis（Channel 层） │
└─────────────────────────────────────────────────────┘

部署方式：开发环境直接运行；生产环境 Docker Compose 一键部署
```
